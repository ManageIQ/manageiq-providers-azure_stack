describe ManageIQ::Providers::AzureStack::CloudManager::OrchestrationTemplate do
  describe '.eligible_manager_types' do
    it 'lists the classes of eligible managers' do
      described_class.eligible_manager_types.each do |klass|
        expect(klass <= ManageIQ::Providers::AzureStack::CloudManager).to be_truthy
      end
    end
  end

  let(:valid_template) { FactoryBot.create(:orchestration_template_azure_stack) }
  ...
  context 'when a raw template in JSON format is given' do
    it 'parses parameters from a template' do
      ...
      assert_allowed_values(param_hash['imageSku'])
    end
  end
  ...
  def assert_secret_type(parameter)
    expect(parameter).to have_attributes(
      ...
      :default_value => 'demoPassword123456',
      :hidden        => true,
      :required      => true,
      :constraints   => []
    )
  ...
  def assert_allowed_values(parameter)
    ...
    expect(constraints[0]).to have_attributes(
      :description    => nil,
      :allowed_values => ['16.04-LTS', '14.04-LTS']
    )
  end
  ...
  describe '#deployment_options' do
    it do
      ...
      assert_deployment_option(options[3], 'deploy_mode', :OrchestrationParameterAllowed, true)
    end
  end
